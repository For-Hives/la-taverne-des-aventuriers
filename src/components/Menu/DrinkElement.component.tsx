...
const mapRecordToDrink = (record: RecordModel, collection_name: string): Drink => {
	return {
...
export default async function DrinkElement({ collection_name, priceDisplay = 'global' }: DrinkElementProps) {
	// Fetching the drink list based on the collection name
	const data = (await getDrinkList(collection_name)) as unknown as DrinkListResponse
	...
	const drinks = data.items.map(record => mapRecordToDrink(record, collection_name))
					...
					<div key={drink.id || drink.title} className="w-full">
						...
						>
							...
							<div className="flex flex-col space-y-4">
								...
								<p className="font-cardo-regular text-custom-brown-100 text-base lg:text-lg">{drink.description}</p>
							</div>
						</div>
					</div>
				))}
